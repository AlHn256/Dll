...
namespace StartTestProject.Forms
{
    public partial class ThreadsNumber : Form
    {
        ...
        private bool LoadingFrDir  = false;
        
        // Загрузка примеров из папки
        private void LoadFrDirBtn_Click(object sender, EventArgs e)
        {
            string DirName = "D:\\Panorama\\Test\\ImgListTrain 12350";

            //FolderBrowserDialog folderBrowserDialog1 = new FolderBrowserDialog();
            //var result = folderBrowserDialog1.ShowDialog();
            //if (result == DialogResult.OK) DirName = folderBrowserDialog1.SelectedPath;

            if (string.IsNullOrEmpty(DirName)) return;
            SettingsDirTxtBox.Text = DirName;
            LoadingFrDir = true;
            //var fileList = fileEdit.SearchFiles(DirName);

            string[] directory = Directory.GetDirectories(DirName);
            if (directory.Length == 0) return;

            Threads = directory.Length;
            //RecreateFiles();
            RedrawBtns();
            CheckFiles();
        }
        ...
        private bool CheckFiles()
        {
            if (LoadingFrDir)
            {
                for (int cam_id = 0; cam_id < Threads; cam_id++)
                {
                    string file = GetDefAssemblingPlanN(cam_id);
                    if (!fileEdit.ChkFile(file)) return SetErr(file + " не создан");

                    AssemblyPlan assemblyPlan = new AssemblyPlan();
                    assemblyPlan.WorkingDirectory = SettingsDirTxtBox.Text + Path.DirectorySeparatorChar + cam_id.ToString("D2");
