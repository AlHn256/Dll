...
namespace ImgAssemblingLibOpenCV.Models
{
    public class FileEdit
    {
        ...
        public string DirFile(string Dir, string File)
        {
            ...
            if (Dir[Dir.Length - 1] == Path.DirectorySeparatorChar) Dir = Dir.Substring(0, Dir.Length - 1);
            if (File[0] == Path.DirectorySeparatorChar) File = File.Substring(1);
        ...
        public string[] GetAutoSaveFilesList(string additionText = null)
        {
            ...
            List<string> AutoSaveFilesList = new List<string>()
            {
                ...
                AdditionalFolder + Path.DirectorySeparatorChar + ApplicationFileName
            };
        ...
        public FileInfo[] SearchFiles(string dir, string[] filter) => SearchFiles(dir, filter, 0);
        ...
        public bool DelAllFileFromDir(string rezultDir, string[] filter)
        {
            FileInfo[] fileList = SearchFiles(rezultDir, filter);
            if (fileList.Length == 0) return false;
            return DelAllFileFromList(fileList);
        }
        public bool DelAllFileFromDir(string rezultDir)
        {
            FileInfo[] fileList = SearchFiles(rezultDir);
            if (fileList.Length == 0) return false;
            return DelAllFileFromList(fileList);
        }
        ...
        public string SaveImg(Mat rezultImg = null, Image DisplayImage = null)
        {
            ...
            {
                if (rezultImg.Height == 0 && rezultImg.Width == 0)
                {
                    SetErr("Err SaveImg.Height & Width = 0 !!!");
                    return string.Empty;
                }
