...
namespace ImgAssemblingLibOpenCV.AditionalForms
{
    ...
    public partial class ImgFixingForm : Form
    {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            else if (keyData == Keys.NumPad2 || keyData == Keys.Down) ChangeC(false);
        ...
        private async void SaveAsBtn_Click(object sender, EventArgs e)
        {
            ...
            //(await fileEdit.SaveJsonAsync(saveFileDialog.FileName, GetImgFixingSettings())) RezultRTB.Text = "Settings save in " + saveFileDialog.FileName;
        ...
        private async Task <bool> SaveSetting(string file)
        {
            try
            {
                if (await fileEdit.SaveJsonAsync(file, GetImgFixingSettings())) 
                    RezultRTB.Text = "Settings save in " + file;
            }
            catch (Exception ex)
            {
        ...
        private void LoadFrBtn_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if(!string.IsNullOrEmpty(ImgFixingFile))SaveSetting(ImgFixingFile);
                }
            }
        }
        ...
        {
            if (File.Exists(ImgFixingFile) || File.Exists(loadingFile))
            {
                if (string.IsNullOrEmpty(loadingFile)) loadingFile = ImgFixingFile;
                else if (!File.Exists(loadingFile)) loadingFile = ImgFixingFile;
            }
            ...
            if (File.Exists(loadingFile))
            {
                try
                {
                    ...
                    //    if (jsonString != null)
                    //    {
                    //        imgFixingSettings = JsonConvert.DeserializeObject<ImgFixingSettings>(jsonString);
        ...
        {
            if(!string.IsNullOrEmpty(ImgFixingFile))await SaveSetting(ImgFixingFile);
        }
