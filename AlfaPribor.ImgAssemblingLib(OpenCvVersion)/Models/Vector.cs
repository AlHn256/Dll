...
namespace ImgAssemblingLibOpenCV.Models
{
    public class Vector
    {
        ...
        //public const int VectorInErrorLimits = 2* PixelError; // Вектор в пределах погрешности
        ...
        public bool inErrorLimits { get; set; }
        #endregion
        public Vector(int matchesId, double xfr, double yfr, double xto, double yto)
        {
            ...
            Shift = Math.Sqrt(dY * dY + dX * dX);
            if (Shift < PixelError) isSamePoint = true;
            //if (Shift < VectorInErrorLimits) inErrorLimits = true;
